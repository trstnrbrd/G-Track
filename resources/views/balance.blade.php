<x-app-layout>
    ...
    <div class="hidden lg:block max-w-7xl mx-auto px-8 py-8">
        ...
        <div class="grid grid-cols-2 gap-5 mb-6">
            <div class="rounded-2xl p-6 text-white shadow-lg relative overflow-hidden" style="background: linear-gradient(135deg, #0066FF, #2B8CFF);">
                <div class="absolute w-40 h-40 rounded-full bg-white/10 -top-12 -right-8"></div>
                <p class="relative text-blue-50 text-sm font-medium mb-1">Current GCash Balance</p>
                <p class="relative text-3xl font-bold">₱{{ number_format($balance->gcash_balance, 2) }}</p>
            </div>
            <div class="rounded-2xl p-6 text-white shadow-lg relative overflow-hidden" style="background: linear-gradient(135deg, #15a34a, #22c55e);">
                <div class="absolute w-40 h-40 rounded-full bg-white/10 -top-12 -right-8"></div>
                <p class="relative text-green-50 text-sm font-medium mb-1">Current Cash On Hand</p>
                <p class="relative text-3xl font-bold">₱{{ number_format($balance->cash_balance, 2) }}</p>
            </div>
        </div>

        {{-- Filters --}}
        <form method="GET" action="{{ route('balance') }}" class="bg-white rounded-2xl shadow-sm p-6 mb-6">
    ...
    <div class="lg:hidden max-w-md mx-auto min-h-screen bg-gray-50 px-5 py-6 pb-safe-nav">
        ...
        <div class="grid grid-cols-2 gap-3 mb-5">
            ...
            <div class="bg-white rounded-2xl shadow-sm p-4">
                <p class="text-gray-400 text-xs font-medium">Cash On Hand</p>
                <p class="text-gray-900 text-lg font-bold mt-0.5">₱{{ number_format($balance->cash_balance, 2) }}</p>
            </div>
        </div>

        {{-- Filters --}}
        <form method="GET" action="{{ route('balance') }}" class="bg-white rounded-2xl shadow-sm p-4 mb-5 space-y-3">
            <div>

...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        {{-- Apply the saved device theme before the page paints, preventing a light flash. --}}
        <script>
            (function () {
                var savedTheme = localStorage.getItem('gtrack-theme') || 'system';
                var isDark = savedTheme === 'dark' ||
                    (savedTheme === 'system' && window.matchMedia('(prefers-color-scheme: dark)').matches);
                document.documentElement.classList.toggle('dark', isDark);

                window.setGTrackTheme = function (theme) {
    ...
    <body class="antialiased bg-canvas">
        ...
        @php
            ...
            $nav = [
                ...
                ['label' => 'History',        'url' => route('history'), 'active' => request()->routeIs('history'),
                 'icon' => 'M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z'],
                ['label' => 'Balance',        'url' => route('balance'), 'active' => request()->routeIs('balance'),
            ...
            if (Auth::user()->isSuperAdmin()) {
                $nav[] = ['label' => 'User Management', 'url' => route('user-management'), 'active' => request()->routeIs('user-management'),
                    'icon' => 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z'];
            }
            $activeLabel = request()->routeIs('settings')
                ? 'Settings'
                : (collect($nav)->firstWhere('active', true)['label'] ?? 'Dashboard');
        ...
        <aside class="hidden lg:flex lg:flex-col fixed inset-y-0 left-0 w-64 bg-surface border-r border-gray-100 z-40">
        ...
        <div class="lg:pl-64 min-h-screen flex flex-col">
            ...
            {{-- Topbar --}}
            <header class="sticky top-0 z-30 h-16 bg-surface/90 backdrop-blur border-b border-gray-100 flex items-center justify-between px-4 lg:px-8">
                <h1 class="text-lg font-bold text-gray-900">{{ $activeLabel }}</h1>

                {{-- Account dropdown --}}
                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open"
                            class="w-9 h-9 rounded-full flex items-center justify-center font-semibold text-white bg-gcash-600 transition hover:bg-gcash-700 focus:outline-none focus:ring-2 focus:ring-gcash-300 focus:ring-offset-2"
                            :class="open ? 'ring-gcash-400' : 'ring-gcash-300'"
                            :aria-expanded="open"
                            aria-haspopup="true">
                    ...
                    <div x-show="open" x-cloak @click.outside="open = false"
                         ...
                         class="absolute right-0 mt-2 w-52 bg-surface rounded-xl shadow-xl ring-1 ring-black/5 py-1.5 z-40 origin-top-right">
                        ...
                        <div class="py-1">
                            {{-- Settings --}}
                            <a href="{{ route('settings') }}" class="group flex items-center gap-2.5 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition">
        ...
        <div x-data="{ moreOpen: false }" class="lg:hidden">
            ...
            <div x-show="moreOpen" x-cloak class="fixed inset-0 z-50">
                ...
                <div x-show="moreOpen"
                     ...
                     class="absolute bottom-0 inset-x-0 bg-surface rounded-t-3xl p-4 pb-safe-sheet">
                    ...
                    @foreach ($moreNav as $item)
                        <a href="{{ $item['url'] }}"
                           class="flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-medium
                                  {{ $item['active'] ? 'bg-gcash-50 text-gcash-700' : 'text-gray-600 hover:bg-gray-50' }}">
            ...
            {{-- Tab bar --}}
            <nav class="fixed bottom-0 inset-x-0 bg-surface border-t border-gray-100 z-40 flex pb-safe">
                @foreach ($primaryNav as $item)
                    <a href="{{ $item['url'] }}"
                       class="relative flex-1 flex flex-col items-center justify-center gap-1 py-2.5 transition
                              {{ $item['active'] ? 'text-gcash-600' : 'text-gray-400' }}">
                        @if ($item['active'])
                            <span class="absolute top-0 w-8 h-[3px] rounded-b-full bg-gcash-600"></span>
                        @endif
                ...
                <button @click="moreOpen = true"
                        class="relative flex-1 flex flex-col items-center justify-center gap-1 py-2.5 transition
                               {{ $moreActive ? 'text-gcash-600' : 'text-gray-400' }}">
        ...
        <div x-data="{ open: false }" @open-logout.window="open = true" x-show="open" x-cloak
            ...
            <div x-show="open"
                 ...
                 class="relative bg-surface rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
        ...
        @if (session('swal'))
            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    Swal.fire(Object.assign({ confirmButtonColor: '#1F5AE0' }, @json(session('swal'))));
                });
            </script>
        @endif
    </body>
